...
We have performed resolution tests to determine if our results are resolution dependent. The important simulation parameter to see how many stars form within a certain spatial range is called StarClusterCombineRadius. Within our original simulation, this parameter was set to 10 pc, meaning that all stars that would form within 10 pc are merged into a single star. To make sure that resolution does not affect our results, we have run simulations for a range of this parameter, namely from 0.625 pc to the original 10 pc. We see the same number of Pop III stars form regardless of the chosen value for the merging parameter. Thus, our results are resolution independent. This detail has been added to section 2.2.1.
