...
The 10 pc quoted above is a parameter called \texttt{StarClusterCombineRadius}, and controls whether newly formed star particles will be merged within a certain radius. Within our original simulation, this parameter was set to 10 pc, meaning that all stars younger than 10 kyr and within 10 pc of each other are merged into a single star particle.  The 10 kyr timescale originates from the approximate protostellar timescale when the ionizing luminosity is linearly increased to its ZAMS value. We chose a value of 10 pc because it corresponds to the approximate radius where H$_2$ formation allows the gas to cool from the halo's virial temperature (e.g. Yoshida et al. 2006).

To ensure that this value does not affect our results, we have run a suite of test simulations for a range of this parameter, namely from 0.625 pc, increasing by a factor of two up to the original 10 pc. The simulations are a 1 comoving Mpc$^3$ box with the same mass and spatial resolution and physics set, zoomed-in on a single halo with a mass of $10^{5.8} M_{\odot}$ at the end of the run. The simulations run for a total of 10 Myr after the first star forms. We see the same number of Pop III stars form regardless of the chosen value for the merging parameter. Thus, we conclude that our results are independent of this value. We have added these details to Section 2.2.1. 
...
We have compared the mass calculations between Rockstar and Machacek+01 and have found that the masses are comparable. The difference between Rockstar halo masses and the Machacek+01 halo mass definition varies by a few percent. At high redshifts, when Pop III stars are forming, $\rho_c = \rho_m$, so our calculations are not affected by this difference, and allows the two mass definitions to be directly compared. 
