...
There are a few minor shortcomings to this work that should be noted. \textbf{As discussed in section 2.2.1, the number of Pop III stars that form is insensitive down to distances of 0.625 pc. Since this value is larger than fragmentation scales, it's possible that the location of the Pop III stars formed in our simulation are sites for multiple Pop III star formation. With this in mind, our results are only strengthened by the possibility of having more Pop III stars forming at these locations.} 
...
\textbf{A more recent study by \citet{Schauer19}, who used cosmological simulations to study cold gas content within halos for varying streaming velocities, showed that high streaming velocities restrict first star formation to atomic cooling halos. Their minimum halo masses which host cold dense gas for various streaming velocities are shown as black horizontal lines with different linestyles in Figure \ref{fig:compare_JLW_mass}. \citet{Hirano17_Science} investigated the effect that streaming velocities have on black hole formation in the early universe and find that in regions with high streaming velocities and for z $> 30$, first generation star formation can be suppressed in minihalos up to the atomic cooling limit, and may produce massive black hole seeds.} 

Our results indicate that halos collapse at larger masses that are not as affected by typical streaming velocities ($<1\sigma_{\rm vbc}$). \textbf{We also find that Pop III stars begin to form at much later times than the halos found in \citet{Hirano17_Science}.} In the evolution of such a halo, it would have started to accumulate gas above these critical masses while also being irradiated by a LW background.  Afterwards, the collapse will be controlled by the processes discussed in Section \ref{sec:variations}, and thus streaming velocities would likely not affect our results. 
